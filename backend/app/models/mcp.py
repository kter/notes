...
class MCPTokenCreateRequest(BaseModel):
    ...
    expires_in_days: Literal[30, 60, 90, 365, None] = Field(
        365, description="Token expiration period in days (30, 60, 90, 365, or None for no expiration)"
    )
...
class MCPTokenListItem(BaseModel):
    ...
    name: str = Field(..., description="Purpose/usage of the API key")
    created_at: str = Field(..., description="ISO format creation timestamp")
    expires_at: str | None = Field(None, description="ISO format expiration timestamp (null for no expiration)")
    ...
    expires_in_days: int | None = Field(None, description="Token expiration period in days (null for no expiration)")
